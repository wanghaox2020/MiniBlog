    ...
    if ($stmt->num_rows > 0){
        while ($row = $stmt->fetch()) {
            ...
            <p class=\"card-text\">%s: %s</p>
                    <div style=\"display: inline-flex;float: right;\">
                        <button style=\"display: flex; width: 100px;height: 37px;\" onclick=\"vote()\" type=\"button\" id=\"voting\" class=\"btn btn-primary\">
            ...
            </div>", $username, $content, $thumb_ups, $post_time);
        }
    }
    printf("<br>");
    printf("<br>");
...
<script>
function vote() {
    var text = document.getElementById('liken').innerHTML;
    var myArray = text.split(" ");
    var value = parseInt(myArray[1], 10);
    value = isNaN(value) ? 0 : value;
    value +=1;
    var s = `Like ${value}`;
    document.getElementById('liken').innerHTML = s;
}
</script>
</html>